...
class Voo implements Reservavel, Serializable {
    ...
    private Aviao aviao; // composição
    public int numero;
    ...
    private Set<String> cpfsRegistrados = new HashSet<>(); // ADIÇÃO: prevenir duplicatas
    private Map<String, Passageiro> mapaPassageiros = new HashMap<>(); // ADIÇÃO: uso de Map
    ...
    public boolean reservar(Passageiro passageiro) {
        if (lotacaoAtual < lotacaoMaxima && !cpfsRegistrados.contains(passageiro.getCPF())) {
            ...
            cpfsRegistrados.add(passageiro.getCPF()); // ADIÇÃO
            mapaPassageiros.put(passageiro.getCPF(), passageiro); // ADIÇÃO
            lotacaoAtual++;
            salvarPassageiros();
            return true;
        }
    ...
    public boolean cancelarReserva(Passageiro passageiro) {
        if (passageiros.remove(passageiro)) {
            cpfsRegistrados.remove(passageiro.getCPF()); // ADIÇÃO
            mapaPassageiros.remove(passageiro.getCPF()); // ADIÇÃO
            lotacaoAtual--;
            salvarPassageiros();
            return true;
        }
        ...
        if (arquivo.exists()) {
            try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(arquivo))) {
                ...
                for (Passageiro p : passageiros) {
                    cpfsRegistrados.add(p.getCPF()); // ADIÇÃO
                    mapaPassageiros.put(p.getCPF(), p); // ADIÇÃO
                }
